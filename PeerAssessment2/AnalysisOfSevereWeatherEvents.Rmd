...
regexThunderstormWind <- "(WIND)|(WND)"
regexHail <- "HAIL"
D <- D[, eventConvection := indicator(paste(regexLightning, regexTornado, regexThunderstormWind, regexHail, sep="|"))]
```
...
```{r}
regex <- "COLD"
D <- D[, eventCold := indicator(regex)]
regex <- "HEAT"
D <- D[, eventHeat := indicator(regex)]
D <- D[, eventExtremeTemp := eventCold == TRUE | eventHeat == TRUE]
```

Create an indicators for variations of **Flash Flood** and **River Flood**.
These fall into the top-level category of **Flood**.

```{r}
regex <- "FLASH"
D <- D[, eventFlashFlood := indicator(regex)]
regex <- "(\\bFLO\\S+?OD\\b)"
D <- D[, eventRiverFlood := indicator(regex) & eventFlashFlood == FALSE]
D <- D[, eventFlood := eventFlashFlood == TRUE | eventRiverFlood == TRUE]
```
